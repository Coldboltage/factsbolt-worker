...
export class JobsService {
  ...
  async factCheckLang({
    ...
    const model = new OpenAI({
      temperature: 0,
      modelName: jobType === JobType.STOCK ? 'gpt-4o' : 'gpt-4o',
    });
    ...
    if (jobType === JobType.STOCK) {
      textContext = await this.utilsService.getAllStockInformation(
        text,
        context,
      );
    } else {
    ...
    const searchTerm = !text
      ? await this.combinedClaimSetup(
          jobType,
          transcriptionJob.text,
          title,
          textContext,
        )
      : await this.combinedClaimSetup(jobType, text, title, textContext);
    ...
    const getDocByClaim = async (
      ...
      // //
      this.logger.verbose(claim, docs);
      return docs;
    };

    const claimPromises = searchTerm.map((claim, index) =>
      getDocByClaim(claim, index),
    );
    ...
    if (jobType === JobType.STOCK) {
      result = await chain.invoke({
        ...
        question: `
          Using the documents as context, provide an extensive analysis of why the following shares ${JSON.stringify(
            !text ? transcriptionJob.utterance : text,
            null,
            2,
          )} are either up or down. Please include:
    
        A broad market analysis to identify any major news or trends affecting the markets associated with the shares. Emphasize the most recent and impactful news and events, as these have a significant impact on market movements, in relation to the share/positions attached. Priorities the things that have impacted the current price/outcome of the stocks/market most recently.

      - Explain the impact of these factors on the market, particularly focusing on any affected industries.
      - Detail any statements or actions by influential political figures by name, that have influenced market volatility.
      - Discuss the effects of major economic policies or trade restrictions on key sectors.
      - Analyze sector volatility, explaining how fluctuations in specific sectors have impacted the shares.
      - Specific reasons for each stock's movement, highlighting if the movement is primarily due to broad market factors or individual stock-specific factors.

      For each stock, provide the following sections:

      1. Reasons: Provide a clear main reason, supported by data and context from the most recent documents, explaining why each stock is performing well or not based on relevant developments. Include a brief insight into how these developments specifically impact the stock's performance.

      2. Outlook: Provide the short term (days to weeks) and long term (years) outlook for the share.

      Emphasize if multiple shares are affected by the same broad market factors, considering this as a major reason, supported by data.

      Provide insights into how the identified factors are likely to affect the shares in the short term (days to weeks), medium term (months), and long term (years).

      Include relevant quotes and general context from the documents to support the analysis without needing specific per share sources.

      The goal is to have a comprehensive understanding with detailed explanations and references, determining if the movements are due to broad market conditions or individual share-specific issues, and backing up each conclusion with evidence.
        `,
      });
    } else {
      result = await chain.invoke({
  ...
  async stockCheckupJob(
    stockCheckupJobDto: StockCheckupJobDto,
  ): Promise<FullTextJob> {
    const completeStockCheckupJob = await this.factCheckLang({
      jobType: JobType.STOCK,
      title: stockCheckupJobDto.title,
      text: stockCheckupJobDto.text,
      context: stockCheckupJobDto.context,
    });

    return {
      chatgpt: completeStockCheckupJob,
      text: {
        text: stockCheckupJobDto.text,
      },
    };
  ...
  async combinedClaimSetup(
    ...
    let listOfClaims: Promise<string[]>;
    let newsContext: Promise<string[]>;
    let transcriptClaims: Promise<string[]>;

    console.log(textContext)

    if (jobType === JobType.STOCK) {
      // newsContext = this.utilsService.relatedContextNews(text);
      // await new Promise((r) => setTimeout(r, 3000000));

      transcriptClaims = this.utilsService.getAllClaimsFromTranscript(
        text,
        title,
        textContext,
        jobType,
        // `Claims must be based upon the most recent information from today's date ${new Date()}. The claim can only be generated if the surrounding context, suggests it to be based upon today's day.`
      );
      // Lets get all the main news relating to the shares
    } else {
      listOfClaims = this.utilsService.getAllClaimsFromTranscript(
        text,
        title,
        textContext,
        jobType,
